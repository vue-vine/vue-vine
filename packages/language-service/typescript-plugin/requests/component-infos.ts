...
import { isVueVineVirtualCode, type VueVineCode } from '@vue-vine/language-service'
...
export function getComponentProps(
  ...
  const vueVineCode = volarFile.generated.root
  const program = languageService.getProgram()!
  const checker = program.getTypeChecker()
  const components = getVariableType(ts, languageService, vueVineCode, '__VINE_VLS_components')
  if (!components) {
    return []
  }
...
export function getTemplateContextProps(
  ...
  return getVariableType(ts, languageService, vueVineCode, '__VINE_VLS_ctx')
    ?.type
    ?.getProperties()
    .map(c => c.name)
}
...
export function getComponentNames(
  ...
  return getVariableType(ts, languageService, vueVineCode, '__VINE_VLS_components')
    ?.type
    ?.getProperties()
    .map(c => c.name)
...
export function _getComponentNames(
  ...
  return getVariableType(ts, tsLs, vueVineCode, '__VINE_VLS_components')
    ?.type
    ?.getProperties()
    .map(c => c.name)
    .filter(entry => !entry.includes('$') && !entry.startsWith('_'))
...
export function getElementAttrs(
  ...
  let tsSourceFile: ts.SourceFile | undefined = program.getSourceFile(fileName)

  if (tsSourceFile) {
    const typeNode = tsSourceFile.statements.find(
      (node): node is ts.TypeAliasDeclaration => (
        ts.isTypeAliasDeclaration(node)
        && node.name.getText() === '__VINE_VLS_IntrinsicElementsCompletion'
      ),
    )
    const checker = program.getTypeChecker()
...
function getVariableType(
  ...
  let tsSourceFile: ts.SourceFile | undefined = program.getSourceFile(vueVineCode.fileName)
