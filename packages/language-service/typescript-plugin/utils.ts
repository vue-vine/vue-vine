...
function waitingForNamedPipeServerReady(namedPipePath: string) {
  ...
  socket.on('connect', () => {
    console.log('[Vue Vine Named Pipe Client] Connected:', namedPipePath, 'in', `${Date.now() - start}ms`)
    socket.write('ping')
  })
  socket.on('data', () => {
    console.log('[Vue Vine Named Pipe Client] Ready:', namedPipePath, 'in', `${Date.now() - start}ms`)
...
export function sendRequestWorker<T>(request: Request, socket: net.Socket) {
  return new Promise<T | undefined | null>((resolve) => {
    ...
    const onData = (chunk: Buffer) => {
      ...
      if (text.endsWith('\n\n')) {
        ...
        try {
          ...
          console.error('[Vue Vine Named Pipe Client] Failed to parse response:', text)
        }
        cleanup()
        resolve(json)
      }
    ...
    const onError = (err: any) => {
      console.error('[Vue Vine Named Pipe Client] Error:', err.message)
      cleanup()
      resolve(undefined)
    }
    const cleanup = () => {
