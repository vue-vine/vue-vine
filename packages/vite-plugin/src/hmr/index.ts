...
export class VineHMRManager {
  ...
  async handleHMR(
    ...
    try {
      ...
      let patchRes: HMRPatchResult | null = null
      ...
      forEachImportedModule((im) => {
        ...
        if (query.type === QUERY_TYPE_SCRIPT) {
          patchRes = analyzeHMRPatch(originalFileCtx, newFileCtx)
          newFileCtx.renderOnly = patchRes.renderOnly
        }
      })
      ...
      // update vineFileCtx and handle HMR
      compilerCtx.fileCtxMap.set(fileId, newFileCtx)
      compilerCtx.isRunningHMR = true
      ...
      // At this point, patchRes is not null
      const patchResTyped: HMRPatchResult = patchRes
      newFileCtx.hmrCompFnsName = patchResTyped.hmrCompFnsName
      const { type } = patchResTyped
